...
## Corpus Check
- 37 files · ~14,421 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 395 nodes · 611 edges · 30 communities (18 shown, 12 thin omitted)
- Extraction: 69% EXTRACTED · 31% INFERRED · 0% AMBIGUOUS · INFERRED: 187 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1573b4de`
...
1. `Agent` - 21 edges
2. `AnthropicAdapter` - 21 edges
3. `GeminiAdapter` - 20 edges
4. `Bus` - 19 edges
5. `BackendCapabilities` - 18 edges
6. `Router` - 17 edges
7. `conflicts()` - 17 edges
8. `TestEnvelope` - 17 edges
9. `TestPayloadModels` - 17 edges
10. `StateGraph` - 16 edges
...
- `main()` --calls--> `StateGraph`  [INFERRED]
  examples/two_agents_conflict_demo.py → sdk-python/synapse/state.py
- `main()` --calls--> `Agent`  [INFERRED]
  examples/two_agents_conflict_demo.py → sdk-python/synapse/agent.py
- `main()` --calls--> `Router`  [INFERRED]
  examples/two_agents_conflict_demo.py → runtime/router/worker.py
- `make_backend()` --calls--> `AnthropicAdapter`  [INFERRED]
  examples/two_agents_with_real_llm.py → sdk-python/synapse/adapters/hosted/anthropic_adapter.py
...
Cohesion: 0.08
Nodes (16): BackendUnavailable, Raised when the backend cannot be reached. SDK falls back to no-coordination mod, AnthropicAdapter, Anthropic hosted adapter — Sonnet/Haiku/Opus via the Anthropic Python SDK.  Impl, Cached-restart injection.          Cancels current stream, then issues a NEW req, Anthropic separates `system` from `messages`. Extract it if present., Hosted-tier adapter for Anthropic.      Capability flags reflect Anthropic's str, GeminiAdapter (+8 more)

### Community 2 - "Community 2"
Cohesion: 0.14
Nodes (27): BaseModel, Enum, str, AgentRegistration, BackendCapabilities, Belief, Block, Conflict (+19 more)

### Community 3 - "Community 3"
Cohesion: 0.08
Nodes (25): InferenceAdapter, InferenceAdapter Protocol — see spec/adapter.md for the canonical contract., Opaque handle to an in-flight generation. Adapter-specific contents., Raised when an operation is requested that the backend does not support     (e.g, StreamHandle, Token, UnsupportedCapability, MockAdapter (+17 more)

### Community 4 - "Community 4"
Cohesion: 0.1
Nodes (15): collect(), main(), make_backend(), Phase 2 deliverable — same conflict demo, real LLM (Gemini by default).  Drop-in, Pick a hosted adapter based on env. Cheap defaults.      Gemini auto-uses Vertex, Read up to N tokens from a streaming handle and return the joined text., _section(), _wait_for_ready() (+7 more)
...
- **Why does `StateGraph` connect `Community 4` to `Community 0`, `Community 3`, `Community 2`, `Community 11`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `Agent` connect `Community 11` to `Community 2`, `Community 3`, `Community 4`, `Community 5`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `Bus` connect `Community 5` to `Community 3`, `Community 2`, `Community 11`, `Community 4`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Are the 12 inferred relationships involving `Agent` (e.g. with `InferenceAdapter` and `Bus`) actually correct?**
  _`Agent` has 12 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 12 inferred relationships involving `GeminiAdapter` (e.g. with `BackendUnavailable` and `InferenceAdapter`) actually correct?**
  _`GeminiAdapter` has 12 INFERRED edges - model-reasoned connections that need verification._
- **Are the 6 inferred relationships involving `Bus` (e.g. with `Router` and `Agent`) actually correct?**
  _`Bus` has 6 INFERRED edges - model-reasoned connections that need verification._