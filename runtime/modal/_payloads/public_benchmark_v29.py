"""Public benchmark v29 — CROSS-FRAMEWORK COOPERATIVE APP BUILD (OpenAI).

Same design as v28 but routes every adapter through OpenAI gpt-4o-mini
since Anthropic + Gemini keys weren't reachable in the launch shell.
...
APP_DIR = "/tmp/v29_app"
SESSION = f"v29_app_{int(time.time())}"
...
async def role_google_adk(content_capture: dict, session: str) -> None:
    ...
    runner = InMemoryRunner(agent=agent, app_name="v29_adk")
    sess = await runner.session_service.create_session(
        app_name="v29_adk", user_id="bench")
...
async def main() -> None:
    import synapse
    print(f"=== v29 CROSS-FRAMEWORK COOPERATIVE APP BUILD (OpenAI) ===")
    ...
    print("\n" + "=" * 90)
    print(f"  v29 SUMMARY: cross-framework cooperative app build (session={SESSION})")
    print("=" * 90)
    ...
    artifact_dir = f"/tmp/v29_artifact_{int(time.time())}"
    ...
    out = f"/tmp/public_benchmark_v29_{int(time.time())}.json"
