...
APP_DIR = "/tmp/v31_app"
SESSION = f"v31_app_{int(time.time())}"
...
async def role_google_adk(content_capture: dict, session: str) -> None:
    ...
    runner = InMemoryRunner(agent=agent, app_name="v31_adk")
    sess = await runner.session_service.create_session(
        app_name="v31_adk", user_id="bench")
...
async def main() -> None:
    import synapse
    print(f"=== v31 CROSS-FRAMEWORK COOPERATIVE APP BUILD (OpenAI) ===")
    ...
    print("\n" + "=" * 90)
    print(f"  v31 SUMMARY: cross-framework cooperative app build (session={SESSION})")
    print("=" * 90)
    ...
    artifact_dir = f"/tmp/v31_artifact_{int(time.time())}"
    ...
    out = f"/tmp/public_benchmark_v31_{int(time.time())}.json"
