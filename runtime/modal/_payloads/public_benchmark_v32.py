...
APP_DIR = "/tmp/v32_app"
SESSION = f"v32_app_{int(time.time())}"
...
async def role_google_adk(content_capture: dict, session: str) -> None:
    ...
    runner = InMemoryRunner(agent=agent, app_name="v32_adk")
    sess = await runner.session_service.create_session(
        app_name="v32_adk", user_id="bench")
...
async def main() -> None:
    import synapse
    print(f"=== v32 CROSS-FRAMEWORK COOPERATIVE APP BUILD (OpenAI) ===")
    ...
    try:
        ...
        for role, info in ROLES.items():
            ...
            if not captured.strip():
                ...
                low = captured.lower()
                missing = []
                if "from flask" not in low: missing.append("from flask")
                if "@app.route" not in captured: missing.append("@app.route")
                if "todos" not in low: missing.append("todos")
                if "jsonify" not in low: missing.append("jsonify")
                # also try py_compile
                try:
                    compile(captured, "<crewai-main.py>", "exec")
                except SyntaxError as ce:
                    missing.append(f"syntax:{str(ce.msg)[:30]}")
                if missing:
                    needs_fallback = True
                    reason = f"main.py invalid: missing {', '.join(missing)} (got {len(captured)}B)"
            if needs_fallback:
                try:
    ...
    print("\n" + "=" * 90)
    print(f"  v32 SUMMARY: cross-framework cooperative app build (session={SESSION})")
    print("=" * 90)
    ...
    artifact_dir = f"/tmp/v32_artifact_{int(time.time())}"
    ...
    # ------------------------------------------------------------------
    # v32: dump the full artifact bundle to STDOUT so framework_sandbox
    # captures it locally (Modal /tmp doesn't persist across runs).
    # ------------------------------------------------------------------
    print("\n" + "=" * 90)
    print("  v32 ARTIFACT BUNDLE (full contents — for local extraction)")
    print("=" * 90)
    # Per-file dumps
    for role, info in ROLES.items():
        path = os.path.join(APP_DIR, info["file"])
        if os.path.isfile(path):
            ...
            print(f"\n>>>>>>>>>> FILE: {info['file']}  ({len(file_content)} bytes, role={role}) <<<<<<<<<<")
            print(file_content)
            print(f"<<<<<<<<<< END {info['file']} <<<<<<<<<<")
    # Envelope JSONL dump
    env_path = os.path.join(artifact_dir, "envelopes.jsonl")
    if os.path.isfile(env_path):
        with open(env_path) as f: env_content = f.read()
        print(f"\n>>>>>>>>>> FILE: envelopes.jsonl  ({len(env_content)} bytes) <<<<<<<<<<")
        print(env_content)
        print(f"<<<<<<<<<< END envelopes.jsonl <<<<<<<<<<")
    print("=" * 90)
    print("  END ARTIFACT BUNDLE")
    print("=" * 90)
    ...
    out = f"/tmp/public_benchmark_v32_{int(time.time())}.json"
